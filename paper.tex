...
\begin{abstract}
\textbf{Methodological contribution.}  We identify and formalize a
general reproducibility pitfall in PSLQ-based integer-relation
searches: raising the target bound (\texttt{maxcoeff}) without also
raising the iteration budget (\texttt{maxsteps}, which defaults to
$100$ in mpmath) leaves the algorithm returning \texttt{None} whether
it has genuinely certified non-existence or has merely run out of
iterations - the two are indistinguishable without inspecting the
algorithm's internal termination state.  We quantify how sharply this
degrades with basis size (a rate of $\sim 0.19$ certified decimal
digits per iteration on a $3$-element basis collapses to
$\sim 0.0004$ on a $26$-element basis and to an outright $0$ on a
$28$-element basis, all under identical settings), show that an
earlier draft of this manuscript fell into exactly this trap on three
separate tests, and demonstrate the fix: independent cross-validation
via LLL lattice reduction (fpylll), which produced certificates in
seconds where the trapped PSLQ runs needed $8.7$-$12.3$ hours to
produce a near-worthless bound.  We believe this diagnostic
(Section~\ref{sec:pitfall}) is of more general use than any specific
bound in this paper.

\textbf{Numerical results.}  We employ two independent
integer-relation engines - mpmath's PSLQ and LLL lattice reduction via
fpylll - to search systematically for algebraic relations between
$\zeta(3)$ and~$\pi$.
...
(7)~we numerically verify the conjectured bases of the multiple zeta
value algebra at every weight $0$-$11$, computing the genuine
generators $\zeta(6,2)$ and $\zeta(8,2)$ (depth~2, trigamma
reduction) and $\zeta(5,3,3)$ (depth~3, an iterated Hurwitz-tail
reduction validated by five independent controls) to $1{,}100$
digits; the weight-8 result targets the open lower-bound direction of
Zagier's dimension conjecture - $\zeta(6,2)$ is not a rational
combination of $\pi^8$, $\zeta(3)^2\pi^2$, $\zeta(3)\zeta(5)$ with
coefficient norm below $10^{237}$ - and the weight-11 test carries a
theorem-guaranteed built-in control that simultaneously validates
evaluator, basis, and search.  Specialized MZV numerics has verified
this structure to far higher weight (the Data Mine of
Bl\"umlein--Broadhurst--Vermaseren reached weight~$22$); ours is an
independent reproduction within this paper's certified-bound
framework, not a frontier advance
(Sections~\ref{sec:zagier}, \ref{sec:weight11}).
All 37 PSLQ tests plus the LLL cross-validation, extended-search,
new-constant, and Zagier-dimension suites are reported with full
parameters; the entire LLL workload runs in about $12$ minutes.
...
In the course of this project, an earlier draft of this manuscript
fell into a reproducibility trap general enough to be worth reporting
on its own terms: three tests reported PSLQ ``certificates'' of
enormous bounds ($10^{50}$-$10^{200}$) that had never actually been
reached, because raising the requested bound
(\texttt{maxcoeff}) without also raising the iteration budget
(\texttt{maxsteps}, which defaults to $100$ in mpmath) leaves the
function returning \texttt{None} regardless of whether it certified
anything or simply ran out of iterations - the two cases are
indistinguishable without checking the algorithm's internal state.  We
formalize this as a general diagnostic
(Section~\ref{sec:pitfall}), quantify how sharply it degrades with
basis size, and demonstrate the fix (independent cross-validation via
LLL, Section~\ref{sec:lll}).  We consider this finding, and not any
particular bound on $\zeta(3)$, the result most likely to be useful to
readers who never intend to think about $\zeta(3)$ again.
...
\subsection{A general diagnostic: the PSLQ certification
pitfall}\label{sec:pitfall}

We believe the most broadly useful finding of this project is not
about $\zeta(3)$ at all, and is worth stating as a general warning
independent of any specific result in this paper.

\textbf{The pitfall.}  mpmath's \texttt{pslq(basis, maxcoeff=M)}
returns \texttt{None} in exactly two circumstances: (a)~the
algorithm's internal norm bound provably exceeded~$M$, a rigorous
certificate that no relation exists with coefficients below~$M$; or
(b)~the algorithm exhausted its iteration budget
(\texttt{maxsteps}, defaulting to $100$) before the norm bound
reached~$M$, in which case \texttt{None} means nothing more than
``not found in $100$ iterations'' - the same as a plain, uncertified
failure.  \textbf{These two outcomes are indistinguishable from the
return value alone.}  An earlier version of this manuscript fell into
exactly this trap: three tests (Remark~\ref{res:deg25-revised})
reported bounds of $10^{50}$-$10^{200}$ that were never actually
reached, because \texttt{maxsteps} was left at its default while
\texttt{maxcoeff} was raised to an astronomical value.  The function
returned \texttt{None} every time, and every time it was silently
wrong to call that a certificate.

\textbf{Severity depends sharply on basis size, in a way that is easy
to underestimate.}  The norm bound grows at a roughly constant rate
\emph{per iteration} for a fixed basis, but that rate collapses as the
basis grows - the same finite precision is divided across more
dimensions:
...
\midrule
$3$ (main test) & $\sim 0.19$ digits/iteration & $\sim 10^{19}$ \\
$26$ (degree-25 algebraicity) & $\sim 0.0004$ digits/iteration & $\sim 10^{0.04}$ (nothing) \\
$28$ (bivariate degree-6) & $0$ (norm $\le 1$ for $3{,}000$ iterations) & nothing \\
$31$ (degree-30 algebraicity) & $\sim 0.0001$ digits/iteration & $\sim 10^{0.01}$ (nothing) \\
\bottomrule
\end{tabular}
\end{center}

A basis of $26$-$31$ elements is not exotic - the original
manuscript's own degree-30 test used exactly this size - and yet the
default settings certify essentially nothing on it.  mpmath's
documentation contains one oblique acknowledgment, in a worked example
rather than a warning attached to the \texttt{pslq} signature itself:
``It may also be necessary to set maxsteps high to prevent a premature
exit (before the coefficient bound has been reached).''  This
correctly implies premature exit is possible, but does not explain
that the resulting \texttt{None} is then silently indistinguishable
from a genuine certificate, nor that the failure rate is
basis-size-dependent in the way quantified above.  We checked
mpmath's issue tracker (open and closed issues mentioning
\texttt{pslq} or \texttt{maxsteps}) and found no prior report of this
specific ambiguity, and have since filed one:
\url{https://github.com/mpmath/mpmath/issues/1130}.

\textbf{This is a real gap in mpmath's implementation specifically,
not an unsolved problem in PSLQ generally.}  The theoretical
distinction between ``norm bound exceeded \texttt{maxcoeff}''
(rigorous) and ``iteration budget exhausted'' (not rigorous) is well
understood in the PSLQ literature (e.g.\ Bailey's papers on the
algorithm~\cite{FergusonBaileyArwade1999} describe both termination
conditions explicitly), and at
least one other PSLQ implementation we are aware of (the Go package
\texttt{github.com/ncw/pslq}) returns a distinct
\texttt{ErrorIterationsExceeded} rather than conflating the two
outcomes into a single ambiguous result.  mpmath's \texttt{pslq} does
not make this distinction available without manually parsing
\texttt{verbose=True} output and comparing the printed iteration count
to \texttt{maxsteps} - which is what we now do.

\textbf{The diagnostic.}  Before treating any
\texttt{pslq(...) is None} result as a certificate, check the
algorithm's actual termination condition (mpmath exposes this via
\texttt{verbose=True}; we implement this check in
\texttt{run\_tests.py} \S9): if the norm bound reached
\texttt{maxcoeff}, the result is rigorous; if \texttt{maxsteps} was
exhausted first, it is not, regardless of what \texttt{maxcoeff} was
requested.

\textbf{The fix we demonstrate.}  LLL reduction
(Section~\ref{sec:lll}) does not share this failure mode in the same
form - a completed LLL reduction always yields a genuine, computable
exclusion bound from the reduced basis, even if that bound is smaller
than hoped.  Where PSLQ needed $8.7$-$12.3$ hours to produce a
near-worthless bound on these bases, LLL produced a bound exceeding
the original (never-achieved) claim in single-digit seconds
(Remark~\ref{res:deg25-revised}).  We recommend, as general practice:
(1)~always verify PSLQ's termination mode rather than trusting a
\texttt{None} return; (2)~treat iteration-budget requirements as
scaling badly, not linearly, with basis size; (3)~cross-validate any
large-basis null result with an independent method before publishing
it as a certificate.
...
\subsection{Zagier's dimension conjecture: verified through weight 10,
targeting the open direction at weights 8 and 10}\label{sec:zagier}

Every result so far in this paper is an exclusion: a relation was
searched for and not found.  This section reports something
structurally different - a verification of the conjectured bases of
the multiple zeta value algebra at every weight through~$10$,
including, at weights~$8$ and~$10$, numerical evidence bearing on the
\emph{open} direction of Zagier's conjecture.

\textbf{Background.}  Multiple zeta values (MZVs) generalize
$\zeta(3)$ to nested sums of arbitrary depth;
$\zeta(s_1,s_2)=\sum_{m>n\ge 1} m^{-s_1}n^{-s_2}$ (we use the
decreasing-index convention throughout this section; the weight-5
test of Section~\ref{sec:mzv} labels its values in the opposite
convention, as parts of the literature do).  Euler proved that every
MZV of weight~$\le 7$ is a polynomial in single zeta values.
Zagier's dimension conjecture~\cite{Zagier1994} predicts the
dimension~$d_w$ of the weight-$w$ graded piece via
$d_w = d_{w-2}+d_{w-3}$ ($d_0=1$, $d_1=0$, $d_2=1$):
$1,0,1,1,1,2,2,3,4,5,7$ for $w=0,\ldots,10$.  Critically, the two
directions of this conjecture have different statuses:
$\dim \le d_w$ is a \emph{theorem} (Terasoma; Deligne--Goncharov),
while $\dim \ge d_w$ - that the conjectured generators are actually
$\Q$-linearly independent - is completely open (even the
irrationality of $\zeta(5)/\pi^5$ is unproven).

\textbf{Weights 0-7 (a theorem, re-confirmed).}  The number of
monomials in $\{\pi^2,\zeta(3),\zeta(5),\zeta(7),\zeta(9),\zeta(11)\}$
at weight~$w$ is a partition count matching $d_w$ exactly for
$w=0$-$7$ and $w=9$, falling short at $w=8$ ($3$ vs.\ $4$) and
$w=10$ ($5$ vs.\ $7$) - the classically known weights where a
generator not expressible in single zetas first appears.  LLL
confirms the monomials are genuinely independent: bounds
$10^{2000.1}$, $10^{2001.0}$, $10^{1333.4}$ at weights $5$, $6$, $7$
respectively ($5{,}000$-digit precision).

\textbf{Weights 8-10 (the genuine tests).}  The conventional extra
generators are $\zeta(6,2)$ at weight~$8$ and $\zeta(8,2)$ at
weight~$10$ - both \emph{depth-2} MZVs (an earlier version of this
manuscript wrongly claimed depth~$\ge 3$ was required and deferred
these tests as infeasible; we record the correction here).  Depth-2
values $\zeta(a,2)$ reduce, via the trigamma identity
$H^{(2)}_{m-1}=\zeta(2)-\psi'(m)$, to
\[
  \zeta(a,2) \;=\; \zeta(2)\,\zeta(a)\;-\;\sum_{m\ge 1}
  \frac{\psi'(m)}{m^a},
\]
whose tail is smooth and Euler--Maclaurin-summable to arbitrary
precision with mpmath's \texttt{nsum} - no dedicated evaluator was
needed after all.  Before use, the evaluator was validated against
three known closed forms ($\zeta(2,2)=\pi^4/120$;
$\zeta(4,2)=\zeta(3)^2-\tfrac43\zeta(6)$;
$\zeta(3,2)=3\zeta(2)\zeta(3)-\tfrac{11}{2}\zeta(5)$), each reproduced
to $400$-plus digits, with LLL independently re-discovering the first
two as identities; $\zeta(6,2)$ was additionally cross-checked against
a brute-force truncated double sum to the expected truncation error.

\begin{result}\label{res:zagier}
With $\zeta(6,2)$ and $\zeta(8,2)$ computed to $1{,}100$ digits and
all other values at matching precision (LLL scale $950$ digits), no
rational relation exists within the certified bounds among the
conjectured Zagier basis at each weight:
\begin{center}
\begin{tabular}{@{}clc@{}}
\toprule
Weight & Conjectured basis ($d_w$ elements) & LLL bound \\
\midrule
8 & $\pi^8,\ \zeta(3)^2\pi^2,\ \zeta(3)\zeta(5),\ \boldsymbol{\zeta(6,2)}$ & $10^{237.7}$ \\
9 & $\zeta(9),\ \zeta(7)\pi^2,\ \zeta(5)\pi^4,\ \zeta(3)\pi^6,\ \zeta(3)^3$ & $10^{189.8}$ \\
10 & $\pi^{10},\ \zeta(3)^2\pi^4,\ \zeta(3)\zeta(5)\pi^2,\ \zeta(3)\zeta(7),\ \zeta(5)^2,\ \boldsymbol{\zeta(6,2)\pi^2},\ \boldsymbol{\zeta(8,2)}$ & $10^{135.2}$ \\
\bottomrule
\end{tabular}
\end{center}
\end{result}

\noindent The weight-$8$ row is the one with real content: if
$\zeta(6,2)$ were a rational combination of the three single-zeta
monomials, the weight-$8$ MZV space would be $3$-dimensional and
Zagier's conjecture false.  Our certificate excludes any such relation
with coefficient norm below $10^{237}$.  Since $\dim\le 4$ is already
a theorem, this is numerical evidence for the open lower-bound
direction: the weight-$8$ space really does look $4$-dimensional.
These are the only results in this paper that bear directly on the
open direction of a named conjecture.

\textbf{Scope relative to prior MZV numerics.}  This verification is
new to this paper, not to the field.  The Multiple Zeta Value Data
Mine of Bl\"umlein, Broadhurst, and Vermaseren~\cite{BBV2010}
numerically verified the conjectured dimension structure of the MZV
algebra to far higher weight (weight~$22$, and in restricted settings
beyond), using dedicated high-performance evaluators much more
capable than the trigamma reduction used here.  Our weight-$10$ reach
is nowhere near that frontier and does not claim to be.  What this
section contributes is (a)~an independent, from-scratch reproduction
inside this paper's certified-exclusion-bound framework, with the
validation chain documented above, and (b)~a demonstration that the
LLL infrastructure built for the $\zeta(3)$-$\pi$ search extends to
structural conjecture-testing at essentially zero marginal cost.
Readers interested in the state of the art in MZV verification should
consult~\cite{BBV2010}, not this section.

\textbf{A cautionary note from this section's own development.}  Our
first run of these tests produced plausible-looking ``relations'' with
$25$-$43$-digit coefficients at every weight.  They were spurious: we
had run LLL with only a $40$-digit margin between the input precision
and the lattice scale, and the spurious-vector filter in
\texttt{lll\_tests.py} needs a margin exceeding ${\sim}50$ digits to
reject balanced no-relation vectors (their coefficient sizes,
$10^{S/n}$, matched the generic prediction exactly, which is what gave
them away).  The filter now raises an error on margins under $100$
digits rather than silently mis-certifying.  We mention this because
it is the LLL-side analogue of the PSLQ pitfall of
Section~\ref{sec:pitfall}: every certification method in this paper
has at least one silent failure mode, and both were caught only by
knowing what the failure signature looks like.

\subsection{Weight 11: the first genuine depth-3
test}\label{sec:weight11}

Weight~$11$ is where depth~$2$ stops sufficing, for a provable
reason: single-zeta monomials plus $\zeta(6,2)\zeta(3)$ supply only
$8$ of the conjectured $d_{11}=9$ dimensions, and by Euler's
classical theorem every depth-2 MZV of odd weight reduces to single
zetas - so the $9$th generator must have depth~$\ge 3$.  We take
$\zeta(5,3,3)$ (all indices $\ge 2$) and extend the Hurwitz-tail
method of Section~\ref{sec:zagier} one level deeper
(\texttt{lll\_weight11.py}): iterating
$H^{(s)}_{m-1}=\zeta(s)-Z(s,m)$ ($Z$ = Hurwitz zeta) through the
triple sum and swapping summation order gives, for indices
$a,b,c\ge 2$,
\[
  \zeta(a,b,c) = \zeta(a)\zeta(b,c) - \zeta(c)A + \zeta(a)B - C,
\]
\[
  A=\sum_m \frac{Z(b,m)}{m^a},\quad
  B=\sum_n \frac{Z(c,n)}{n^b},\quad
  C=\sum_n \frac{Z(c,n)\,Z(a,n{+}1)}{n^b},
\]
all smooth Euler--Maclaurin-summable sums.  Five validations precede
any use: (1)~$\zeta(2,2,2)=\pi^6/5040$ (the classical
$\zeta(\{2\}^n)=\pi^{2n}/(2n{+}1)!$) reproduced to $400$-plus digits;
(2)~a brute-force truncated triple sum for $\zeta(5,3,3)$ agrees to
exactly the predicted truncation error; (3)~LLL finds the
Euler-guaranteed weight-7 reduction of $\zeta(3,2,2)$, namely
$[80,-785,100,-2]$ against
$\{\zeta(3,2,2),\zeta(7),\zeta(5)\pi^2,\zeta(3)\pi^4\}$; (4)~LLL
finds the known weight-8 relation between $\zeta(5,3)$ and
$\zeta(6,2)$; (5)~LLL finds the weight-8 reduction of the depth-3
value $\zeta(4,2,2)$ in the basis containing $\zeta(6,2)$.

\textbf{A theorem-guaranteed control is built into the main test.}
Since $\dim\le 9$ at weight~$11$ is a theorem, any \emph{ten}
weight-11 elements of the MZV algebra must satisfy a rational
relation.  We therefore run two searches: the $9$-element conjectured
basis (where the open direction of Zagier's conjecture predicts no
relation), and the same basis plus a second depth-3 value,
$\zeta(7,2,2)$, where a relation \emph{must} exist and LLL finding
it - with small, clearly non-spurious coefficients - validates the
evaluator, the basis, and the search simultaneously.

\begin{result}\label{res:weight11}
At $1{,}100$-digit precision (LLL scale $950$), the $9$-element
conjectured weight-11 basis
\[
  \{\zeta(11),\ \zeta(9)\pi^2,\ \zeta(7)\pi^4,\ \zeta(5)\zeta(3)^2,\
  \zeta(5)\pi^6,\ \zeta(3)^3\pi^2,\ \zeta(3)\pi^8,\
  \zeta(6,2)\zeta(3),\ \boldsymbol{\zeta(5,3,3)}\}
\]
admits no rational relation with coefficient norm below
$10^{104.3}$, while the $10$-element extension by
$\boldsymbol{\zeta(7,2,2)}$ yields the theorem-required relation
$[1057995,\ -56700,\ -846,\ -64800,\ -320,\ 0,\ 0,\ 64800,\ 25920,\ -32400]$.
\end{result}

\noindent The control relation's coefficients ($7$ digits at most,
against $\sim 10^{95}$-scale spurious vectors at this lattice size)
leave no doubt it is the genuine theorem-guaranteed identity; the
identical integer vector was independently found in a $450$-digit
prototype run, a two-precision cross-confirmation - in
effect, a numerically discovered depth-3 reduction of $\zeta(7,2,2)$
into the conjectured basis.  The $9$-element null result extends this
paper's evidence for the open lower-bound direction of Zagier's
conjecture from weight~$10$ to weight~$11$, now including a generator
no depth-2 method can reach.  The same scope caveat as
Section~\ref{sec:zagier} applies: the Data Mine~\cite{BBV2010}
verified far beyond this weight.  Weight~$12$ requires two further
generators, at least one of depth~$4$ with an index equal to~$1$
(classically $\zeta(6,4,1,1)$), which breaks the Hurwitz-tail
reduction used here; that is where we stop.
...
\subsection{The PSLQ certification pitfall as a general finding}

We consider Section~\ref{sec:pitfall} the most transferable
contribution of this paper.  It arose directly from a failure: an
earlier draft claimed three PSLQ ``certificates'' that had never
actually been computed, because the trap it describes - raising
\texttt{maxcoeff} without raising \texttt{maxsteps} - produces a
\texttt{None} return indistinguishable from a genuine certificate
unless the algorithm's termination mode is checked.  This is not
specific to $\zeta(3)$ or to this codebase: any use of mpmath's
\texttt{pslq} that reports a bound without recording how many
iterations were needed to reach it is vulnerable to the same silent
failure, and the severity (Section~\ref{sec:pitfall}) grows sharply
worse, not gracefully, as basis size increases.  We want to be precise
about what is and is not new here.  The theoretical distinction
between the two termination conditions is well understood in the
PSLQ literature, and is not a gap in the algorithm itself; at least
one other implementation we found (the Go package
\texttt{github.com/ncw/pslq}) already surfaces it via a distinct
error type rather than a bare \texttt{None}.  What we did not find,
after searching mpmath's issue tracker and documentation, is any
existing report that mpmath's own implementation conflates the two
outcomes in exactly this way.  We report the gap as specific to this
widely-used implementation, not as a previously-unknown property of
PSLQ in general - and we do not know whether it has caused unverified
claims elsewhere, only that the same silent success (the function call
returns cleanly and looks correct either way) would make such claims
easy to miss.  We have since filed this as
\texttt{mpmath/mpmath\#1130}
(\url{https://github.com/mpmath/mpmath/issues/1130}), so it is now on
record upstream regardless of what becomes of this paper.

The practical fix we demonstrate - cross-validating any large-basis
or high-bound PSLQ null result against LLL - costs seconds where the
trapped PSLQ runs cost hours, and does not share the same failure
mode.  We recommend it as standard practice for any PSLQ-based
exclusion result intended for publication, independent of anything
else in this paper.

\subsection{Interpretation of the $\zeta(3)$ results}
...
number-theoretic structures.  We are also careful not to overstate the
value of a larger bound in isolation: excluding coefficients below
$10^{18695}$ is not meaningfully stronger evidence than excluding them
below $10^{18}$ was, since no plausible hidden relation was ever
expected to have an $18$-digit coefficient any more than an
$18{,}695$-digit one.  The exercise demonstrates that the search method
scales essentially without limit given compute; it does not, by
itself, bring the open question closer to resolution.

\subsection{A result on an open conjecture: Zagier through weight 10}

Section~\ref{sec:zagier} is the one place in this paper where the
results bear on the open direction of a named conjecture rather than
excluding relations no one expected.  At weights $0$-$7$ the tests
re-confirm Euler's classical reduction theorem.  At weights $8$
and~$10$, however, the conjectured bases require the depth-2
generators $\zeta(6,2)$ and $\zeta(8,2)$, and verifying their
independence from the single-zeta monomials is precisely the
direction of Zagier's dimension conjecture that is unproven: the
upper bound $\dim\le d_w$ is a theorem (Terasoma;
Deligne--Goncharov), but no lower bound is known - it is not even
proven that $\zeta(5)/\pi^5$ is irrational.  Our certificate that
$\zeta(6,2)$ is not a rational combination of $\pi^8$,
$\zeta(3)^2\pi^2$, $\zeta(3)\zeta(5)$ (coefficient norm below
$10^{237}$) is numerical evidence that the weight-8 space has the
full conjectured dimension~$4$.  An honest deflationary caveat
applies here as everywhere in this paper: a finite exclusion bound is
not a proof, and no number of computed digits can close the
lower-bound direction.  But unlike the $\zeta(3)$-$\pi$ exclusions,
where no serious conjecture predicted a relation, here the
computation probes exactly the statement the field cannot yet prove.
A second deflationary caveat is scope: specialized MZV numerics has
verified this structure to far higher weight than we reach - the
Data Mine of Bl\"umlein, Broadhurst, and
Vermaseren~\cite{BBV2010} went to weight~$22$ - so our contribution
is an independent reproduction within this paper's verified
framework, not an advance of the field's frontier
(Section~\ref{sec:zagier}).

A process note: an earlier version of this manuscript claimed these
weight-8-plus tests were infeasible without a dedicated
depth-$\ge 3$ MZV evaluator.  That was wrong on both counts - the
needed generators are depth~2, and they reduce via a trigamma
identity to sums mpmath handles directly
(Section~\ref{sec:zagier}).  The correction is recorded there,
alongside a second implementation lesson: our first run of these
tests returned spurious ``relations'' caused by running LLL with too
thin a precision margin, now guarded against in code.
...
Zagier 1994: MZV dimension conjecture \cite{Zagier1994}
  & Independent reproduction through weight~$10$ via LLL, incl.\
  computed $\zeta(6,2)$, $\zeta(8,2)$ (Section~\ref{sec:zagier}); the
  MZV Data Mine~\cite{BBV2010} reached weight~$22$ with dedicated
  tools \\
\bottomrule
\end{tabular}
\end{center}
...
The weight-11 barrier flagged in an earlier version of this
section - the $9$th generator cannot be depth~$2$, by Euler's
odd-weight reduction theorem - has since been crossed:
Section~\ref{sec:weight11} extends the Hurwitz-tail evaluator to
depth~$3$ and completes the weight-11 verification with
$\zeta(5,3,3)$ computed from scratch.  The frontier for this project
is now weight~$12$, which requires two further generators including
one of depth~$4$ with an index equal to~$1$ (classically
$\zeta(6,4,1,1)$); index-$1$ entries break the Hurwitz-tail
reduction used here, so weight~$12$ would need a genuinely different
evaluator (e.g.\ the H\"older-convolution or multiple-polylogarithm
methods used by GiNaC and the Data Mine~\cite{BBV2010}).  Other future non-algebraic tests include
continued-fraction analysis of $\zeta(3)/\pi^3$, numerical
verification of integral representations, and targeted series
identities mixing $\zeta(3)$ and~$\pi$.
...
\textbf{The most transferable result of this paper is methodological,
not numerical.}  We identified, quantified, and fixed a general
reproducibility pitfall in PSLQ-based integer-relation searches - the
indistinguishability of ``certified'' from ``ran out of iterations''
in a bare \texttt{None} return - and we found it the hard way, by
publishing three unverified bounds ourselves before catching it.  We
report the fix (Section~\ref{sec:pitfall}, LLL cross-validation) as a
recommendation for any PSLQ-based work, independent of $\zeta(3)$.
...
\end{itemize}
These bounds far exceed any known identity in zeta function theory.  As
discussed in Section~\ref{sec:discussion}, we do not read a larger
bound as materially stronger evidence than a smaller one already
provided; the exclusion regime here has no precedent in number theory
either way.

\textbf{One line of results targets an open conjecture rather than
excluding the unexpected:} we computed the MZV generators
$\zeta(6,2)$ and $\zeta(8,2)$ (depth~2) and $\zeta(5,3,3)$
(depth~3) to $1{,}100$ digits from scratch, each behind its own
validation chain, and verified the full conjectured Zagier bases at
weights $8$ through~$11$.  The two strongest points: $\zeta(6,2)$ is
not a rational combination of $\pi^8$, $\zeta(3)^2\pi^2$,
$\zeta(3)\zeta(5)$ below coefficient norm $10^{237}$
(Section~\ref{sec:zagier}), and the weight-11 test passes alongside
a built-in theorem-guaranteed control that simultaneously validates
evaluator, basis, and search (Section~\ref{sec:weight11}).  The
field's specialized tooling has verified this structure to
weight~$22$~\cite{BBV2010}; weight~$12$ (needing depth~$4$ with
index-$1$ entries) is where our methods stop
...
\texttt{lll\_new\_constants.py} ($\sim 71$ seconds).  The Zagier
verification through weight~$10$ (Section~\ref{sec:zagier}) is
reproduced by \texttt{lll\_zagier\_check.py} ($\sim 7$ minutes,
dominated by the $1{,}100$-digit evaluations of $\zeta(6,2)$ and
$\zeta(8,2)$).  The weight-11 test with the depth-3 generator
(Section~\ref{sec:weight11}) is reproduced by
\texttt{lll\_weight11.py} ($\sim 35$-$40$ minutes, dominated by the
two $1{,}100$-digit depth-3 evaluations).  Every result reported in
this paper is generated by one of those six scripts.
