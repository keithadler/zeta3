...
(2)~the claim that $\zeta(3)/\pi^3$ is not algebraic of degree~$\le 30$
with polynomial height~$\le 10^{100}$ ($4{,}500$ digits) is
\textbf{withdrawn}: re-verification ($12.3$ hours of compute) produced
a certified norm bound of only $2$, not $10^{100}$ - see
Remark~\ref{res:deg25-revised};
(3)~the claim that $\zeta(3)$ and~$\pi$ satisfy no joint polynomial of
total degree~$\le 6$ is likewise \textbf{withdrawn}: re-verification
produced a certified norm bound of exactly $0$ (no certificate at all;
see Remark~\ref{res:biv6-withdrawn});
(4)~no linear relation connects $\zeta(3)$, $\zeta(3,2)$, $\zeta(2,3)$,
and~$\pi^5$ at weight~5.
Of~37 PSLQ tests, 31 yield certified null results, 3 recover known
identities, and three (degree-25 and degree-30 algebraicity of
$\zeta(3)/\pi^3$, and bivariate degree-6 independence) are
inconclusive or withdrawn - large basis size ($26$+ elements)
prevented PSLQ from reaching a meaningful certified bound in practical
time (Remark~\ref{res:deg25-revised}
...
\begin{result}[Non-algebraicity of $\zeta(3)/\pi^3$ -
\textbf{withdrawn}]\label{res:B}
The original manuscript claimed that at $4{,}500$-digit precision,
$\zeta(3)/\pi^3$ is not algebraic of degree~$\le 30$ with polynomial
height~$\le 10^{100}$.  This was never actually achieved: mpmath's
PSLQ defaults to $100$ iterations, and a run reaching height $10^{100}$
was never completed.  We re-ran this test properly - at $50{,}000$-digit
precision using a full $3{,}000$-iteration budget ($12.3$ hours of
compute) - and the certified norm bound reached only $2$: we can only
certify the absence of a degree-30 relation with coefficients~$\le 2$,
a completely uninteresting bound.  This is the same large-basis
dilution problem affecting the degree-25 and bivariate degree-6 tests
(Remarks~\ref{res:deg25-revised} and~\ref{res:biv6-withdrawn}), here
at its worst since the $31$-element basis is the largest in this
paper.  We withdraw this claim.
\end{result}

\noindent For context: $\zeta(2)/\pi^2=1/6$ is rational (degree~0).
Whether $\zeta(3)/\pi^3$ is algebraic of any degree remains open; the
tests in this paper only meaningfully constrain it at
degree~$\le 10$ and degree~$\le 15$ (Section~\ref{sec:algebraicity}),
not degree~$\le 30$ as originally claimed.
...
The degree~$\le 10$ and degree~$\le 15$ tests (Result~\ref{res:alg-ratio};
$11$- and $16$-element bases) have not been individually re-verified
against the same failure mode, though their more modest bounds and
smaller bases make them less likely to be affected than the
$26$-plus-element tests.

\textbf{A faster path forward (planned re-verification).}  The runs
above used mpmath's pure-Python big-integer backend.  We have since
benchmarked a $28.1\times$ speedup ($0.371$s/iteration vs.\
$10.43$s/iteration on the identical degree-25 workload) from
installing \texttt{gmpy2}, which mpmath auto-detects and uses as a
compiled GMP backend with no code changes required.  This does not
change the norm-growth-per-iteration rate - that is a property of the
basis, not the backend - but it makes far more iterations affordable
in the same wall-clock time: our ``$12$ days for a meaningful
degree-25 bound'' estimate becomes roughly $10$ hours with this
backend.  We plan to re-attempt all three withdrawn/inconclusive tests
with \texttt{gmpy2} and substantially larger iteration budgets, and
will update this paper with the results.  Until then, the withdrawals
in this section stand as the best currently-verified status of these
tests.  A compiled LLL implementation such as \texttt{fpylll} remains
a longer-term alternative that may scale better still; we leave that
...
the bounds we were able to actually verify.  Across 31 of 34
independent tests, at precisions up to $20{,}000$ digits (extended to
$40{,}000$ digits for Result~\ref{res:A-extended}), every PSLQ
computation returned a certified null result.  Three tests involving
large bases ($26$+ elements) do not: the degree-25 and degree-30
algebraicity tests are withdrawn or inconclusive
(Remark~\ref{res:deg25-revised}), and the bivariate degree-6
independence test is withdrawn entirely, having produced no
certificate at all (Remark~\ref{res:biv6-withdrawn}).  Notably, the
degree-30 result was one of this paper's original two headline claims
and does not survive re-verification.

The one remaining principal exclusion is:
\begin{itemize}
  \item $\zeta(3)\ne(a/b)\,\pi^2+c/d$ with coefficients up to
    $10^{18695}$.
\end{itemize}
This bound far exceeds any known identity in zeta function theory.

A formal proof of algebraic independence requires theoretical methods
beyond computation.  Our results establish that if a linear relation
$a\,\zeta(3)+b\,\pi^2+c=0$ exists, it must have coefficients exceeding
$10^{18695}$ - a regime with no precedent in number theory.  We can no
longer make a comparably strong claim about the \emph{degree} of a
possible relation between $\zeta(3)$ and~$\pi$: the original
degree~$\le 30$ exclusion is withdrawn, and the tests that do survive
re-verification (Section~\ref{sec:algebraicity}) only meaningfully
constrain degree~$\le 15$.

\medskip
\noindent\textbf{Any relation $a\,\zeta(3)+b\,\pi^2+c=0$ between
$\zeta(3)$ and~$\pi$, if it exists, must involve coefficients larger
than~$10^{18695}$.  We make no comparably strong claim about
higher-degree algebraic relations - see Remark~\ref{res:deg25-revised}
for what was withdrawn and why.}
