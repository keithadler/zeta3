...
Of~37 PSLQ tests, 33 yield certified null results, 3 recover known
identities, and one (degree-25 algebraicity of $\zeta(3)/\pi^3$) is
inconclusive - a large basis size prevented PSLQ from reaching a
meaningful certified bound in practical time (Remark~\ref{res:deg25-revised}).
All tests are reported with full parameters.  The accompanying
...
the tested bounds.  Across 33 of 34 independent tests, at precisions
up to $20{,}000$ digits (extended to $40{,}000$ digits for
Result~\ref{res:A-extended}), every PSLQ computation returned a
certified null result.  The remaining test (degree-25 algebraicity of
$\zeta(3)/\pi^3$) is inconclusive due to its large basis size
(Remark~\ref{res:deg25-revised}).
