...
These are, to our knowledge, the only results in this paper that bear
directly on the open direction of a named conjecture.
...
Zagier 1994: MZV dimension conjecture \cite{Zagier1994}
  & Conjectured bases verified through weight~$10$ via LLL, incl.\
  computed $\zeta(6,2)$, $\zeta(8,2)$; weight-8 targets the open
  lower-bound direction (Section~\ref{sec:zagier}) \\
\bottomrule
\end{tabular}
\end{center}
...
bound is a theorem (Section~\ref{sec:zagier}).  Extending past
weight~$10$ requires a depth-$\ge 3$ MZV evaluator and is the natural
continuation (Section~\ref{sec:limitations}).
